async function search(value) {
  const response = await fetch("https://civitai.com/api/v1/models?limit=20&sort=Most Downloaded&query=" + value);
  const json = await response.json();
  document.querySelectorAll(".image-card").forEach((card) => card.remove());

  json.items.forEach((item) => {
    const result = document.createElement("article");
    result.classList.add("image-card");
    let innerHTML = `<div class="maintain-aspect-ratio"><div class="images hidden" onclick="showDetails(this, event);" >`;
    item.modelVersions.sort((a, b) => a.createdAt - b.createdAt).reverse();
